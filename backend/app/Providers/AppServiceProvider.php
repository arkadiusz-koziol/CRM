...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        // Export configuration binding
        $this->app->bind(ExportConfig::class, function ($app) {
            $config = $app->make('config');

            return new ExportConfig(
                storagePath: $config->get('export.storage.path', 'exports'),
                maxFileSize: $config->get('export.max_file_size', 50 * 1024 * 1024),
                chunkSize: $config->get('export.chunk_size', 1000),
                fileTtl: $config->get('export.file_ttl', 7),
                logoPath: $config->get('export.logo_path', 'assets/logo.png'),
                footerText: $config->get('export.footer_text', 'Generated by CRM System'),
            );
        });
